  ...
  for (tile_x = ppu_shows_background_in_leftmost_8px() ? 0 : 1; tile_x < 32;
    ...
    for (x = 0; x < 8; x++) {
      ...
      if (color != 0) {
        ...
        draw((tile_x << 3) + x - ppu.PPUSCROLL_X + (mirror ? 256 : 0),
             ppu.scanline + 1, idx);  // bg
        // printf("ppu_draw_background_scanline\n");
      }
    }
  }
}
    ...
    for (x = 0; x < 8; x++) {
      ...
      if (color != 0) {
        ...
        if (PPU_SPRRAM[n + 2] & 0x20) {
          ...
          // printf("draw(screen_x, sprite_y + y_in_tile + 1, idx); // bbg\n");
        } else {
          draw(screen_x, sprite_y + y_in_tile + 1, idx);  // fg
          // printf("draw(screen_x, sprite_y + y_in_tile + 1, idx); // fg\n");
        }
...
void ppu_cycle() {
  ...
  if (ppu.scanline < SCR_H && ppu_shows_sprites()) {
    ...
    // printf("ppu.scanline = %d, %s; ppu_shows_sprites() = %d\n", ppu.scanline,
    //        (ppu.scanline < SCR_H ? "true" : "false"), (int)ppu_shows_sprites());
    // printf("ppu.PPUMASK = 0x%08X\n", ppu.PPUMASK);
  }
  // printf("ppu.PPUMASK = 0x%08X\n", ppu.PPUMASK);
...
#ifdef PROFILE
    uint32_t total =
        cpu_time + background_time + sprite_time + time_diff(t1, t0);
...
void ppuio_write(word address, byte data) {
  ...
  switch (address) {
    ...
    case 1:
      if (ppu.ready) {
        ppu.PPUMASK = data;
        // printf("set ppu.PPUMASK = 0x%08X\n", data);
      } else {
        // printf("cannot set ppu.PPUMASK = 0x%08X\n", data);
      }
      break;
    case 3:
      ppu.OAMADDR = data;
